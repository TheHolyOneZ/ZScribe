...
use ts_rs::TS;
use zscribe_platform::{Hotkey, HotkeyBackend};
...
pub enum RegisterError {
    #[error("{0}")]
    Invalid(#[from] zscribe_platform::HotkeyError),
    ...
    #[error(
        "this session cannot register global hotkeys; bind a key in your compositor to the \
         command `zscribe record` instead"
    )]
    Unsupported,
}
    ...
    manager
        .on_shortcut(shortcut, move |_app, _shortcut, event| {
            ...
            if event.state() == ShortcutState::Pressed {
                crate::recording::toggle(handle.clone());
            }
        })
        .map_err(|err| {
...
fn spawn_portal_session(app: &AppHandle, hotkey: &Hotkey) {
    ...
    let task = tauri::async_runtime::spawn(async move {
        let result = zscribe_platform::portal::run(
            &hotkey,
            move |bound| {
                *bound_handle
            ...
            move || crate::recording::toggle(activate_handle.clone()),
        )
        .await;
